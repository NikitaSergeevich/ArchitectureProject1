...
public class SourceFilterTemplate extends FilterFramework {
    ...
    public void run() {
        ...
        while (true) {
            ...
            /*************************************************************
             * The programmer can insert code for the filter operations here to include reading the
             * data from some device or file. Note that regardless how the data is read, data must
             * be sent one byte at a time out the output pipe. This has been done to adhere to the
             * pipe and filter paradigm and provide a high degree of portability between filters.
             * However, you must convert input data to byte type on your own. The following line of
             * code will write a byte of data out the filter's output port. If you break from the
             * loop you should call ClosePorts() to close the filter ports in an orderly way. This
             * is shown below, but commented out. Where you close the ports will depend upon how you
             * terminate the loop.
             **************************************************************/
